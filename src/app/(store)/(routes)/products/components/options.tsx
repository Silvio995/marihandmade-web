...
import type { BrandType, CategoryType } from '@/types/product'
...
export function AvailableToggle({ initialData }: { initialData?: string }) {
   const router = useRouter()
   const pathname = usePathname()
   const searchParams = useSearchParams()
   ...
   useEffect(() => {
      setValue(initialData === 'true')
   }, [initialData])

   return (
      <div className="flex w-full items-center space-x-2 rounded-md border">
